module bitbucket.org/calmisland/payment-lambda-funcs

go 1.12

require (
	bitbucket.org/calmisland/go-server-account v1.13.0
	bitbucket.org/calmisland/go-server-api v0.2.2
	bitbucket.org/calmisland/go-server-auth v1.1.1
	bitbucket.org/calmisland/go-server-aws v1.14.0
	bitbucket.org/calmisland/go-server-cloud v0.4.0
	bitbucket.org/calmisland/go-server-configs v1.1.1
	bitbucket.org/calmisland/go-server-iap v0.3.3
	bitbucket.org/calmisland/go-server-logs v1.2.1
	bitbucket.org/calmisland/go-server-messages v1.2.0
	bitbucket.org/calmisland/go-server-product v1.9.0
	bitbucket.org/calmisland/go-server-requests v1.14.1
	bitbucket.org/calmisland/go-server-utils v1.9.0
	github.com/calmisland/go-errors v0.9.2
	github.com/calmisland/go-testify v1.3.3
)
