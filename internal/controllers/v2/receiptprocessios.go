...
func ProcessReceiptIos(c echo.Context) error {
	...
	if err != nil {
		...
		case appstore.ErrInvalidJSON, appstore.ErrInvalidReceiptData:
			helpers.LogFormat(contextLogger, "[IAPPROCESSRECEIPT] Received a receipt with invalid format for store [apple] and transaction [%s]", transactionID)
		...
		case appstore.ErrReceiptUnauthenticated, appstore.ErrReceiptUnauthorized:
			helpers.LogFormat(contextLogger, "[IAPPROCESSRECEIPT] Received an unauthorized receipt for store [apple] and transaction [%s]", transactionID)
		...
		case appstore.ErrServerUnavailable:
			helpers.LogFormat(contextLogger, "[IAPPROCESSRECEIPT] The ios verification server is currently unavailable.")
	...
	if productPurchase == nil {
		helpers.LogFormat(contextLogger, "[IAPPROCESSRECEIPT] Unable to find transaction [%s] in receipt for store [apple]", transactionID)
	...
	helpers.LogFormat(contextLogger, "[IAPPROCESSRECEIPT] Successfully processed transaction [%s] for store [apple], store product [%s] and account [%s].", transactionID, storeProductID, accountID)
	return nil
}
